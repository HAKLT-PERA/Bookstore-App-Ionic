...
export class AuthService {
  ...
  private token: string;
	...
	public request(method: 'post' | 'get', url: string, isSecure: boolean, obj?: any): Observable<any> {
		...
		if (method === 'post') {
			if (isSecure) {
				base = this.http.post(this.conf.url+ `/api/${url}`, obj, { headers: { Authorization: `Bearer ${this.getToken()}` } });
			} else {
				base = this.http.post(this.conf.url+`/api/${url}`, obj);
			}
		} else {
			base = this.http.get(this.conf.url+`/api/${url}`, { headers: { Authorization: `Bearer ${this.getToken()}` } });
		}
